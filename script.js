...
const tierIds = ["HT1-items","HT2-items","HT3-items","HT4-items","HT5-items",
                 "LT1-items","LT2-items","LT3-items","LT4-items","LT5-items"];

let currentMode = "sword"; // default
...
function saveTierList() {
  ...
  db.ref(`tierlist/${currentMode}`).set(data)
    .then(() => alert(`${currentMode} tier list saved!`))
    .catch(err => alert("Error: " + err));
}

function loadTierList() {
  db.ref(`tierlist/${currentMode}`).get().then(snapshot => {
    if (!snapshot.exists()) {
      tierIds.forEach(id => document.getElementById(id).innerHTML = "");
      return;
    }
